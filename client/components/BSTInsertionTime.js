...
const BSTInsertionTime = (props) => {
    ...
    return (
    <div>
        <h3 className="runtime">Operation took <span className="runtimeInline">{bstArrLength}</span> moves. Expected <span className="runtimeInline">{moves}</span> moves based on <span className="runtimeInline">{bigO}</span> upper-bound time complexity. </h3>
    </div>
  )
}
